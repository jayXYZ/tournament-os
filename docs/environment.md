...
| `PLATFORM_FEE_PERCENT`       | no         | Platform's cut as a percentage of the entry fee (default `5`). With `STRIPE_FEE_PERCENT`/`STRIPE_FEE_FIXED_CENTS` (defaults `2.9`/`30`, the estimated Stripe processing fee), feeds the fee math in `@tournament-os/shared/payment-fees`; orders snapshot their breakdown at creation, so changes never reprice open orders. |
