...
| ------------------------------------------------------------------------------------------------------------------------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VITE_CONVEX_URL`                                                                                                                          | yes      | Convex deployment URL. Local: from `packages/backend/.env.local` after `pnpm dev:backend`. Production: injected by `convex deploy --cmd-url-env-var-name VITE_CONVEX_URL` in `apps/web/vercel.json` — do not set it manually in Vercel. |
...
written by `pnpm dev:backend` when linking a dev deployment; it is machine-local
